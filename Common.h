...
namespace my {
    ...
    */
    ...
    {
        std::string directoryName = my::Null<std::string>();
        ...
        if (!path.empty())
        {
            ...
            if (delimiter != std::string::npos)
                directoryName = path.substr(0, delimiter + 1);
        }
    ...
    */
    ...
    inline std::string ReplaceKeyword(std::string inputString, std::string keyword, std::string value)
    {
        ...
        while (index != std::string::npos)
        {
            ...
            // BUG: (09-Sep-2018) THE VALUE MAY CONTAIN THE KEYWORD, SO WE SHOULD LOOK FOR NEW KEYWORDS ONLY AFTER THE VALUE.
            index = outputString.find(keyword, index + value.size());
        }
